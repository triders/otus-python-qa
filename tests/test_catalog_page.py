...
def test_if_no_products_in_category(catalog_id, browser, base_url):
    catalog_page = CatalogPage(catalog_id=catalog_id, browser=browser, base_url=base_url)
    catalog_page.open()
    if not catalog_page.is_element_present(catalog_page.LOCATORS["products on page"]):
...
def test_switch_products_view(catalog_id, browser, base_url):
    catalog_page = CatalogPage(catalog_id=catalog_id, browser=browser, base_url=base_url)
    catalog_page.open()
    if catalog_page.is_element_present(catalog_page.LOCATORS["products on page"]):
        # grid view is by default
        assert catalog_page.is_element_present(catalog_page.LOCATORS["products on page grid"])
        assert not catalog_page.is_element_present(catalog_page.LOCATORS["products on page list"])
        # switch to list view
        catalog_page.click(catalog_page.LOCATORS["list view button"])
        assert catalog_page.is_element_present(catalog_page.LOCATORS["products on page list"])
        assert not catalog_page.is_element_present(catalog_page.LOCATORS["products on page grid"])
